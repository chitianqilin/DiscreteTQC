...
if __name__ == "__main__":
    ...
    while env_step < args.total_timesteps:
        ...
        for collect_actor, n_to_add in [(actor, 20), (best_actor, 0)]:
            ...
            for t in range(n_to_add):
                ...
                if terminated or truncated:
                    need_init = True
                    print("losses/result", result, env_step)
                    writer.add_scalar("losses/result", result, env_step)
                    txt.write(f"train {result} {train_step} {env_step}\n")
        ...
        for _ in range(10):
            if rb_mgr.n_sample > args.learning_starts:
                ...
                act_probs = actor.forward(b_obs).detach()
                act = mix * a3 + (1 - mix) * act_probs
                ...
                if train_step % args.policy_frequency == 0:
                    ...
                    entropy_loss = (entropy_loss / (action_dim * np.log(action_atom))).clamp_max(torch.linspace(1.0, 0.0, pmfs.shape[1], device=device).view(1, -1))
