...
o_symmetry = [
    [ 8,  9, 10, 11, 16, 17, 18, 31, 32, 33, 34, 39, 40, 41],
    [12, 13, 14, 15, 19, 20, 21, 35, 36, 37, 38, 42, 43, 44]
]
...
if __name__ == "__main__":
    ...
    def obs_symmetry(_obs: np.ndarray):
        obs = _obs.copy()
        for i, j in zip(*o_symmetry):
            obs[i], obs[j] = obs[j], obs[i]
        return obs
    
    def act_symmetry(_act: np.ndarray):
        act = _act.copy()
        for i, j in zip(*a_symmetry):
            act[i], act[j] = act[j], act[i]
        return act
    
    def prob_symmetry(_prob: np.ndarray):
        prob = _prob.reshape(action_dim, action_atom).copy()
        for i, j in zip(*a_symmetry):
    ...
    while env_step < args.total_timesteps:
        ...
        for collect_actor, n_to_add in [(actor, 10), (best_actor, 0)]:
            ...
            for t in range(n_to_add):
                ...
                cursor = rb_mgr.add()
                rb_obs[cursor]      = torch.tensor(obs_symmetry(obs)).float().to(device)
                rb_next_obs[cursor] = torch.tensor(obs_symmetry(next_obs)).float().to(device)
                rb_prob[cursor]     = torch.tensor(prob_symmetry(to_prob)).float().to(device)
                rb_act[cursor]      = torch.tensor(act_symmetry(to_action)).to(device)
                rb_reward[cursor]   = torch.tensor(reward).float().to(device)
                rb_done[cursor]     = torch.tensor(float(done)).to(device)
